...
class LevelsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(tableView: UITableView, didUpdateFocusInContext context: UITableViewFocusUpdateContext, withAnimationCoordinator coordinator: UIFocusAnimationCoordinator) -> Void {
        ...
        if (context.previouslyFocusedIndexPath != nil) {
            guard let prevCell = self.tableView.cellForRowAtIndexPath(context.previouslyFocusedIndexPath!) as? LevelTableViewCell else {
                return
            }
            prevCell.becomeOnFocus(false)
        }
        if (context.nextFocusedIndexPath != nil) {
            guard let nextCell = self.tableView.cellForRowAtIndexPath(context.nextFocusedIndexPath!) as? LevelTableViewCell else {
                return
            }
            nextCell.becomeOnFocus(true)
        }
    }
}